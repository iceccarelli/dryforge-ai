 ...
 * -----------------------------------------------------------------------------
 * AWS product-page pattern, scaled to the full asset library: series tabs →
 * lazy-loaded masonry grid of all 80 concept renders → full-screen lightbox
 * with prev/next across the active filter.
 *
 * HONESTY POLICY (see README / fleet.ts): DryForge is pre-launch. These are
 * concept renders / design targets, not photographs of deployed hardware. The
 * UI says so, and specs are stated at the series level rather than invented
 * per image.
 */
...
const SERIES_BADGE: Record<SeriesId, string> = {
  independent: "bg-orange-100 text-orange-800 border-orange-200",
  field: "bg-sky-100 text-sky-800 border-sky-200",
};

// Series-level design-target specs (disclosed, not per-image fabrication).
...
function Lightbox({
  ...
  const series = seriesOf(img.series);
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`${series.name} — concept render ${index + 1} of ${items.length}`}
    >
      <motion.div
      ...
      >
        ...
        <div className="relative flex min-h-0 flex-1 items-center justify-center bg-slate-950 lg:w-3/5">
          <div className="relative h-[44vh] w-full sm:h-[56vh] lg:h-[90vh] lg:max-h-[90vh]">
            <AnimatePresence mode="wait">
              ...
              >
                <Image
                  src={img.src}
                  alt={`${series.name} — DryForge concept render ${index + 1}`}
                  fill
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-6 sm:p-8 lg:w-2/5">
          <span
            className={`inline-flex w-fit items-center rounded-full border px-3 py-0.5 text-xs font-semibold tracking-wide ${SERIES_BADGE[img.series]}`}
          >
            {series.name}
          </span>
...
function Tile({
  ...
  const series = seriesOf(item.series);
  return (
    <button
      type="button"
      onClick={onOpen}
      aria-label={`Open ${series.name} concept render`}
    ...
    >
      <div className="relative aspect-[3/4] w-full">
        <Image
          src={item.src}
          alt={`${series.name} — DryForge concept render`}
          fill
          sizes={GRID_SIZES}
          loading="lazy"
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-950/60 via-transparent to-transparent opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
      <span
        className={`pointer-events-none absolute left-2.5 top-2.5 rounded-full border px-2 py-0.5 text-[10px] font-semibold tracking-wide opacity-0 transition-opacity duration-200 group-hover:opacity-100 ${SERIES_BADGE[item.series]}`}
      >
        {series.name}
      </span>
...
export default function FinishingShowcase() {
  ...
  const items = useMemo(
    () => (filter === "all" ? IMAGES : IMAGES.filter((i) => i.series === filter)),
    [filter]
  );

  const tabs: { id: Filter; label: string; count: number }[] = useMemo(
    () => [
      { id: "all", label: "All views", count: IMAGES.length },
      ...SERIES.map((s) => ({
        id: s.id as Filter,
        label: s.name,
        count: IMAGES.filter((i) => i.series === s.id).length,
      })),
    ],
    []
  );
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-lg leading-snug text-slate-600">
            A full library of concept renders of the DryForge platform — taping,
            mudding, and sanding under supervision on real interior geometry. Filter
            the gallery and open any frame for a closer look.
          </p>
        </div>

        {/* Series tabs */}
        <div
          className="df-rail mt-8 flex gap-2 overflow-x-auto pb-1"
          role="tablist"
          aria-label="Filter concept renders by series"
        >
          {tabs.map((t) => {
