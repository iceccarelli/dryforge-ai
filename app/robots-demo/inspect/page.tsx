...
const PRESETS: Preset[] = [
  ...
  { name: "Top", pos: [0.01, 4.4, 0.6], target: [0, 0.9, 0.2] },
  { name: "Tool head", pos: [0.9, 1.3, 2.0], target: [0, 1.2, 0.5] },
];
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-400">{active.name} · run the task, explode it, cut away, inspect every part.</p>
          </div>
          <span
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
          <div className="relative h-[62vh] min-h-[440px] overflow-hidden rounded-2xl border border-slate-800 bg-[#0b1220]">
            <Canvas shadows camera={{ position: [2.8, 1.9, 3.0], fov: 42 }} dpr={[1, 1.85]}>
              ...
              <RobotInternals
                ...
                speed={speed}
              />
              <Grid args={[16, 16]} cellColor="#1e293b" sectionColor="#334155" fadeDistance={24} infiniteGrid />
              <ContactShadows position={[0, 0.005, 0]} opacity={0.55} scale={12} blur={2.4} far={4.5} />
              <OrbitControls ref={controls} enableDamping minDistance={1.4} maxDistance={11} maxPolarAngle={Math.PI / 2.05} target={[0, 0.9, 0.2]} />
            </Canvas>
          ...
          <aside className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">Slide the section plane through the machine.</p>
            </div>
